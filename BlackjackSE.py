...
def blackjack(name='Mr. Bond',new=True):
    ...
    if chips >= 1000:
        ...
        print
        print "Next time, enter your name as 'Sterling Archer'"
    print
...
def bscore(hand):
    ...
    #Different Scores are contained in a list, incremented using lambda calculus
    for i in range(len(hand)):
        if type(hand[i])==int: score += hand[i]
        elif hand[i] == 'Ace': score += 1
        
    return score % 10

def baccarat(new=True):
    ##Welcome
    bgraphic()
    print 
    print 'Welcome ' + (new==False)*'back ' + 'to Punto Banco, Monsieur Archer.'
    #First round gives option to read the rules
    while new:
        rule = raw_input('Would you like to read the rules? O/N: ')
        if rule.upper() == 'O':
            brules()
            break
        elif rule.upper() == 'N':
    ...
    chips = 4000000
    cont = 'Y'
    print 'Vous avez ' + (new==False)*'à nouveau ' +'4000000 chips.'
    ...
    while cont.upper() != 'N':
        while True:
            ...
            ##Place Bet
            print 'Place your bet. It must be in multiples of 25000' \
                  ' between 50000 and %i.' \
                  % chips
            #Restrict bets to integer format
            while True:
                try:
                    bet = int(raw_input('Your bet: '))
                    if bet >= 50000 and bet <= chips and bet % 25000 == 0:
                        print '\nYou bet',bet,'chips.'
                        break
                    else: print 'That is not a valid bet, monsieur.'
                except ValueError:
                    print 'That is not a valid bet, monsieur.'
            ...
            deal(playerhand,deck)
            deal(dealerhand,deck)
            print 'You have',playerhand,'worth %i.' % bscore(playerhand)

            ##PlayerTurn
            if bscore(playerhand) >= 6:
                if bscore(playerhand) >= 8:
                    print 'Natural.'
                    three = False
            else:
                deal(playerhand,deck)
                print 'You draw %s.' % playerhand[2]
                print 'You have %i.' % bscore(playerhand)
                three = True
            playerscore = bscore(playerhand)
            sleep(1)

            ##Dealer Turn
            dealerscore = bscore(dealerhand)
            print '\nHouse has',dealerhand,'worth %i.' % bscore(dealerhand)
            if dealerscore <= 3:
                deal(dealerhand,deck)
                print 'House draws %s.' % dealerhand[2]
                dealerscore = bscore(dealerhand)
            elif dealerscore == 4 and playerscore >= 2 and playerscore <= 7:
                deal(dealerhand,deck)
                print 'House draws %s.' % dealerhand[2]
            elif dealerscore == 5 and playerscore >= 4 and playerscore <= 7:
                deal(dealerhand,deck)
                print 'House draws %s.' % dealerhand[2]
            elif dealerscore == 6 and playerscore >= 6 and playerscore <= 7 \
                 and three:
                deal(dealerhand,deck)
                print 'House draws %s.' % dealerhand[2]
            else:
            ...
            ##Resolution
            ##Evaluate winnings of each hand
            print '\nYou have %i. House has %i.' %(playerscore,dealerscore) 
            sleep(0.3)
        ...
        ##Option to Continue
        print 'Vous avez %i chips, Monsieur Archer.' % chips
        if chips <= 50000:
            sleep(1.5)
            cont = 'N'
        elif chips > 0:
            while True:
                cont = raw_input('Continuez? O/N: ')
                if cont.upper() == 'O' or cont.upper() == 'N': break
                else: print "---Please enter 'O' or 'N'---"
        print
    ...
    if chips <= 25000:
        sadgraphic()
        print "S'il vous plaît appelez 1-800-GAMBLER."
    elif chips >= 10000000:
        print
        for i in range(50):
            print '$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$'
        print
        print 'Bravo, Monsieur Archer, bravo!'
...
def brules():
    print '\nPunto Banco est comme Baccarat.\n'
